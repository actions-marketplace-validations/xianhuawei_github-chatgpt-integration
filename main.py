    ...
    for commit in commits:
        ...
        for file in files:
            ...
            content = repo.get_contents(filename,
                                        ref=commit.sha).decoded_content
            ...
            try:
                ...
                pull_request.create_issue_comment(
                    f"ChatGPT's review about `{file.filename}` file:\n {response['choices'][0]['message']['content']}"
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exclude-dir', help='exclude dir', default="")
    parser.add_argument('--dev-lang',
                        help='Development language used for this request')
    parser.add_argument(
        '--openai-engine',
        default="gpt-3.5-turbo",
        help=
        'GPT-3.5 model to use. Options: text-davinci-003, text-davinci-002, text-babbage-001, text-curie-001, text-ada-001'
    )
    parser.add_argument(
        '--openai-temperature',
        default=0.0,
        help=
        'Sampling temperature to use. Higher values means the model will take more risks. Recommended: 0.5'
    )
    parser.add_argument(
        '--openai-max-tokens',
        default=4096,
